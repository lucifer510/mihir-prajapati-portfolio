...
// BUG FIX: useState was used below but never imported — added here
...
// BUG FIX: each category maps to an ARRAY of projects, so the type must be Project[], not Project
  ...
  "Data Analytics": [
    {
      ...
      live: "https://www.linkedin.com/posts/mihir-prajapati-fs_powerbi-weatherdashboard-dataanalytics-activity-7344980899001741312-PEze?utm_source=share&utm_medium=member_desktop&rcm=ACoAADzjgU4Bi4-dBjCV6AD6TLjRnM2F2EWNFTM",
    ...
    },
    {
      ...
      live: "https://www.linkedin.com/posts/mihir-prajapati-fs_powerbi-hranalytics-datavisualization-activity-7334919165692002305-qago?utm_source=share&utm_medium=member_desktop&rcm=ACoAADzjgU4Bi4-dBjCV6AD6TLjRnM2F2EWNFTM",
    ...
    },
    {
      ...
      live: "https://www.linkedin.com/posts/mihir-prajapati-fs_powerbi-dataanalytics-healthcareanalytics-activity-7337359629909118976-m-Np?utm_source=share&utm_medium=member_desktop&rcm=ACoAADzjgU4Bi4-dBjCV6AD6TLjRnM2F2EWNFTM",
    ...
    },
    {
      ...
      live: "https://www.linkedin.com/posts/mihir-prajapati-fs_powerbi-dataanalytics-dashboarddesign-activity-7332265191264387073-OZid?utm_source=share&utm_medium=member_desktop&rcm=ACoAADzjgU4Bi4-dBjCV6AD6TLjRnM2F2EWNFTM",
    ...
    },
    {
      ...
      live: "https://www.linkedin.com/posts/mihir-prajapati-fs_powerbi-dataanalytics-dashboarddesign-activity-7347534812959682561-CfJW?utm_source=share&utm_medium=member_desktop&rcm=ACoAADzjgU4Bi4-dBjCV6AD6TLjRnM2F2EWNFTM",
    ...
    },
    {
      ...
      live: "https://www.linkedin.com/posts/mihir-prajapati-fs_powerbi-datavisualization-zomato-activity-7350033320409296896-o6em?utm_source=share&utm_medium=member_desktop&rcm=ACoAADzjgU4Bi4-dBjCV6AD6TLjRnM2F2EWNFTM",
...
// BUG FIX: removed ~250 lines of commented-out duplicate legacy data that used to sit here

export default function ProjectsSection() {
  // Currently selected filter pill; "All" shows every category grouped together
  const [activeCategory, setActiveCategory] = useState<string>("All");

  // Filter bar options: "All" plus one button per category key
  const categories = ["All", ...Object.keys(projectData)];

  // When filtering by a single category, flatten isn't needed — grab that category's array directly
  const filteredProjects =
    activeCategory === "All"
      ? Object.values(projectData).flat()
      : projectData[activeCategory] || [];

  // Stagger children fade-in for the section wrapper
  ...
  // Rise + scale-in animation used by the CTA block
  const itemVariants = {
  ...
  // Slow ambient drift for the decorative background blobs
  const floatVariants = {
    animate: {
      y: [-10, 10, -10],
      x: [-5, 5, -5],
  ...
  return (
    ...
    >
      {/* Decorative floating background blobs, purely visual */}
      ...
      >
        <SectionHeader
          ...
          description="Discover my latest work in web development, and data analytics."
          showUnderline={true}
          centered={true}
        />

        {/* Category filter pills */}
        <div className="flex flex-wrap gap-2 mb-6 max-w-7xl mx-auto px-3 md:px-4">
        ...
        {/*
          BUG FIX: this used to be followed by a second, unfinished duplicate
          conditional block (broken ternary + empty map). Removed entirely —
          this single block already covers both the "All" and filtered cases.
        */}
        {activeCategory === "All" ? (
          // Grouped-by-category view: one heading per category, projects stacked underneath
          <div className="space-y-16 max-w-7xl mx-auto px-3 md:px-4">
            {Object.entries(projectData).map(([categoryName, projects]) => (
          ...
          // Single-category view: just the filtered list, no category headings needed
          <div className="space-y-12 max-w-7xl mx-auto px-3 md:px-4">
...
// ======================================================
// BUG FIX: previously this ignored its own `project`/`index` props and instead
// re-looped over the ENTIRE projectData object internally, meaning every card
// rendered every project again inside itself. It now renders only the single
// `project` passed to it, and it's declared at the top level (not inside JSX).
    ...
    >
      <div className="bg-white/80 dark:bg-gray-900/80 backdrop-blur-xl rounded-2xl md:rounded-3xl border border-white/30 dark:border-gray-700/40 hover:border-gray-300 dark:hover:border-gray-600 transition-all duration-500 overflow-hidden shadow-lg md:shadow-xl hover:shadow-xl md:hover:shadow-2xl">
        ...
        >
          ...
          {/* Floating decorative tech icons, hidden on small screens */}
          ...
          <div className="relative z-10">
            {/* Category / status / timeline / client-type badges */}
            ...
            {/* Live demo link (GitHub link intentionally omitted, as in the original) */}
        ...
        {/* Details: features, deliverables, tech stack */}
